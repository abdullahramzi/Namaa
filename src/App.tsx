...
import { Language, Currency, Service, Course, Project, ServiceCategory, SiteSettings, InfoPage, Review } from './types';
import { TRANSLATIONS, SERVICE_CATEGORIES, COURSE_CATEGORIES, PROJECT_CATEGORIES, SERVICES, COURSES, PROJECTS, DEFAULT_INFO_PAGES } from './constants';
...
const App: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'home' | 'services' | 'courses' | 'projects' | 'admin' | string>('home');
  ...
  const handleAddReview = (itemId: string, reviewData: Omit<Review, 'id' | 'date'>) => {
    ...
    // Update locally for modal immediate feedback if needed, 
    // though ItemDetailModal usually relies on props. 
    // Ideally we re-fetch selectedItem from the updated lists, but simple prop passing works if parent updates.
  ...
  const handlePurchase = () => {
    alert(lang === 'ar' ? 'تمت عملية الشراء بنجاح (محاكاة)' : 'Purchase successful (Simulated)');
      ...
      <section className="relative bg-white dark:bg-slate-800 rounded-3xl p-8 md:p-12 shadow-2xl overflow-hidden border border-gray-100 dark:border-slate-700">
         <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative hidden lg:block">
               ...
               <img 
                 src="https://images.unsplash.com/photo-1553877615-30c73e63b4aa?auto=format&fit=crop&q=80&w=800" 
                 alt="Hero" 
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-gray-50 text-gray-900'}`} dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => setViewMode('home')}>
               ...
               <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-500 dark:text-slate-400 font-medium tracking-widest uppercase">
                     {t.appName[lang]}
                  </span>
               </div>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 ml-4">
               ...
               <CurrencySwitcher currency={currency} onToggle={toggleCurrency} lang={lang} />
               <LanguageSwitcher currentLang={lang} onToggle={toggleLanguage} />
